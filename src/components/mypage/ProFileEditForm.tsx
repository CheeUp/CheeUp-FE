    ...
    <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-md shadow-md">
      <div>
        ...
        <input
          ...
          className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:bg-primary file:text-white"
        />
      </div>
...
<div className="mb-6">
  ...
  <div className="flex flex-wrap gap-2 mt-2">
    {selectedStacks.map((stack) => (
      <div
        key={stack}
        className="relative bg-primary text-white group-hover:text-white px-4 py-2 rounded-md cursor-pointer group"
        onClick={() =>
          setSelectedStacks((prev) => prev.filter((s) => s !== stack))
        }
      >
        {/* 기술 스택 이름 */}
        <span className="group-hover:opacity-0 transition-opacity">{stack}</span>
